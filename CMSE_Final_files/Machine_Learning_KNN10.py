...
def create_vectors():
    ...
    return feature_vector, target_vector, win_data, totals

#Trains Data
def train(size):
    feature_vector,target_vector, win_data, totals = create_vectors()
    x_train, x_test, y_train, y_test = train_test_split(feature_vector,target_vector, test_size=size)
    return x_train, x_test, y_train, y_test, feature_vector, target_vector, win_data, totals

#Calculates prediction
def predict(k,size,x_input=[]):
    x_train, x_test, y_train, y_test, feature_vector, target_vector, win_data, totals = train(size)
    scaler = StandardScaler()
    scaler.fit(x_train)
    
    if x_input == []:
        x_t1 = scaler.transform(x_train)
